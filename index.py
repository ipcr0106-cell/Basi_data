...
def set_korean_font():
    try:
        if platform.system() == 'Windows':
            ...
            plt.rcParams['font.family'] = 'AppleGothic'
        else: # Linux/Cloud 환경 (Streamlit Cloud 등)
            # 서버 환경에 폰트가 없을 경우를 대비해 기본 폰트 설정
            plt.rcParams['font.family'] = 'NanumGothic'
    except:
        plt.rcParams['font.family'] = 'sans-serif'
    plt.rcParams['axes.unicode_minus'] = False
...
# --- 3. 데이터 로드 및 전처리 (캐싱 적용) ---
@st.cache_data
def load_and_process_data():
    file_trade = "baci_85_only.csv"
    file_country = "country_codes_V202501.csv"
    
    # 파일 존재 확인
    if not os.path.exists(file_trade) or not os.path.exists(file_country):
        return None, "CSV 파일을 찾을 수 없습니다. baci_85_only.csv와 country_codes_V202501.csv 파일이 소스코드와 같은 폴더에 있는지 확인해주세요."

    try:
        # 데이터 읽기
        baci_hs = pd.read_csv(file_trade)
        baci_country = pd.read_csv(file_country)
        
        # 컬럼명 통일 및 병합
        baci_country = baci_country.rename(columns={'country_code': "j"})
        baci_final = pd.merge(baci_hs, baci_country, on="j", how="left")
        
        # t(연도) 랜덤 생성 (요청 사항: 2021~2023)
        np.random.seed(42)
        years = [2021, 2022, 2023]
        baci_final['t'] = np.random.choice(years, size=len(baci_final))
        
        return baci_final, None
    except Exception as e:
        return None, f"데이터 처리 중 오류 발생: {e}"

# --- 4. 트리맵 레이아웃 알고리즘 (가중치 기반 분할) ---
def treemap_layout(values, x, y, w, h):
    if len(values) == 0: return []
    if len(values) == 1: return [(x, y, w, h)]
...
def main():
    ...
    year_options = sorted(df['t'].unique())
    selected_years = st.sidebar.multiselect("분석 연도 선택", year_options, default=year_options)
    top_n = st.sidebar.slider("트리맵 표시 국가 수", 5, 20, 15)
    ...
    if filtered_df.empty:
        st.warning("선택한 연도에 해당하는 데이터가 없습니다.")
        return
    ...
    with col1:
        ...
        # t를 문자열로 바꿔서 X축 간격 조절
        bars = ax1.bar(yearly_total.index.astype(str), yearly_total.values, color='skyblue', edgecolor='black')
        ax1.set_title('연도별 수출액 합계 (단위: 1,000 USD)')
    ...
    # [우측] 트리맵 국가 비중 분석
    with col2:
        st.subheader(f"🌏 국가별 수출 비중 (상위 {top_n}개국)")
        top_data = filtered_df.groupby('country_name')['v'].sum().sort_values(ascending=False).head(top_n)
